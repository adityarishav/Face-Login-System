...
class UserSystem:
    ...
    def register_user(self, user_id, login_key, frame, role="normal", name=""):
        ...
        if frame is None:
            print("[ERROR] No face data (frame) provided for registration.")
            return False
    ...
    def login_user(self, login_key, live_frame):
        if not login_key:
            print("[ERROR] Login Key cannot be empty.")
            return False
        
        if live_frame is None:
            print("[ERROR] No face data (live_frame) provided for login.")
            return False
