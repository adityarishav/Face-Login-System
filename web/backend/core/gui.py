...
class FaceLoginGUI(customtkinter.CTk):
    ...
    def create_user_dashboard_frame(self):
        ...
        self.activity_log_textbox.configure(state="disabled")
    ...
    def create_change_login_key_frame(self):
        ...
        back_button = customtkinter.CTkButton(self.change_login_key_frame, text="Back to Dashboard", command=self.show_user_dashboard_frame, width=200, height=50, font=("Segoe UI", 16, "bold"))
        back_button.pack(pady=10)
    ...
    def _admin_register_user_threaded(self, user_id, name, login_key, role):
        try:
            if self.user_system.register_user(user_id, login_key, role, name):
                ...
                messagebox.showerror("Error", "Registration failed. User ID might already exist or face data capture failed.")
                self.show_admin_register_frame()
        except Exception as e:
            messagebox.showerror("Error", f"Registration failed: {e}")
    ...
    def populate_admin_recent_logins():
