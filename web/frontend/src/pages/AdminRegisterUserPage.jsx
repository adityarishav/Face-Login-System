...
function AdminRegisterUserPage() {
  ...
  const [role, setRole] = useState('normal'); // Admin registers normal users by default
  ...
  const handleRegister = async () => {
    ...
    photo.toBlob(async (blob) => {
      try {
        ...
        setHasPhoto(false);
        clearPhoto();
        // Optionally navigate back to admin dashboard after successful registration
  ...
  return (
    <div>
      ...
      <button onClick={handleRegister} disabled={loading}>Register User</button>
      <button onClick={() => navigate('/admin-dashboard')} disabled={loading}>Back to Dashboard</button>
    </div>
  );
}
