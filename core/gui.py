...
class FaceLoginGUI(customtkinter.CTk):
    def __init__(self):
        ...
        self.create_change_login_key_frame()
    ...
    def create_admin_dashboard_frame(self):
        ...
        # Admin Profile Card
        admin_profile_card = customtkinter.CTkFrame(self.admin_dashboard_frame, corner_radius=10)
        admin_profile_card.pack(pady=20, padx=20, fill="x")

        customtkinter.CTkLabel(admin_profile_card, text="Admin Profile", font=("Segoe UI", 20, "bold")).pack(pady=10)
        self.admin_name_label = customtkinter.CTkLabel(admin_profile_card, text="", font=("Segoe UI", 16))
        self.admin_name_label.pack(pady=10)

        # Logout Button INSIDE Admin Profile Card
        ...
        change_key_button = customtkinter.CTkButton(self.admin_dashboard_frame, text="Change Login Key", command=self.show_change_login_key_frame, width=200, height=50, font=("Segoe UI", 16, "bold"))
        change_key_button.pack(pady=10)

        # Recent Logins Box for Admin
        recent_logins_label = customtkinter.CTkLabel(self.admin_dashboard_frame, text="Recent Logins (All Users):", font=("Segoe UI", 14, "bold"))
        recent_logins_label.pack(pady=10)

        self.admin_recent_logins_frame = customtkinter.CTkScrollableFrame(self.admin_dashboard_frame, width=400, height=150)
        self.admin_recent_logins_frame.pack(pady=5, padx=20, fill="x", expand=True)
    ...
    def create_user_dashboard_frame(self):
        ...
        change_key_button = customtkinter.CTkButton(security_section, text="Change Login Key", font=("Segoe UI", 14, "bold"), command=self.show_change_login_key_frame)
    ...
    def create_change_login_key_frame(self):
        self.change_login_key_frame = customtkinter.CTkFrame(self.inner_frame, width=500, height=700)
        self.change_login_key_frame.pack_propagate(False)

        label = customtkinter.CTkLabel(self.change_login_key_frame, text="Change Login Key", font=("Segoe UI", 24, "bold"))
        label.pack(pady=20)

        self.old_login_key_entry = customtkinter.CTkEntry(self.change_login_key_frame, placeholder_text="Old Login Key", show="*", width=250, height=30)
        self.old_login_key_entry.pack(pady=10)

        self.new_login_key_entry = customtkinter.CTkEntry(self.change_login_key_frame, placeholder_text="New Login Key", show="*", width=250, height=30)
        self.new_login_key_entry.pack(pady=10)

        self.confirm_new_login_key_entry = customtkinter.CTkEntry(self.change_login_key_frame, placeholder_text="Confirm New Login Key", show="*", width=250, height=30)
        self.confirm_new_login_key_entry.pack(pady=10)

        change_key_button = customtkinter.CTkButton(self.change_login_key_frame, text="Change Key", command=self.change_login_key_threaded, width=200, height=50, font=("Segoe UI", 16, "bold"))
        change_key_button.pack(pady=20)

        back_button = customtkinter.CTkButton(self.change_login_key_frame, text="Back to Dashboard", command=self.back_to_dashboard, width=200, height=50, font=("Segoe UI", 16, "bold"))
        back_button.pack(pady=10)
    ...
    def show_admin_dashboard_frame(self):
        ...
        self.populate_admin_recent_logins()
    ...
    def show_user_dashboard_frame(self):
        ...
        last_login_time = "N/A"
        timestamps = self.user_system.get_user_login_timestamps(user['user_id'])
        if timestamps:
            last_login_time = sorted(timestamps, reverse=True)[0]
        ...
        activity_logs = self.user_system.get_user_login_timestamps(user['user_id'])
        self.activity_log_textbox.configure(state="normal") # Enable editing to insert text
        self.activity_log_textbox.delete("1.0", "end")
        if activity_logs:
            for log in sorted(activity_logs, reverse=True):
                self.activity_log_textbox.insert("end", f"Login: {log}\n")
        else:
            self.activity_log_textbox.insert("end", "No recent login activity.\n")
    ...
    def show_change_login_key_frame(self):
        user = self.user_system.get_logged_in_user()
        if not user:
            messagebox.showerror("Error", "Access denied.")
            self.show_main_frame()
            return
        self.hide_all_frames()
        self.old_login_key_entry.delete(0, 'end')
        self.new_login_key_entry.delete(0, 'end')
        self.confirm_new_login_key_entry.delete(0, 'end')
        self.change_login_key_frame.pack(fill="both", expand=True)
    ...
    def hide_all_frames(self):
        ...
        if hasattr(self, 'change_login_key_frame'):
            self.change_login_key_frame.pack_forget()
    ...
    def create_user_card(self, parent_frame, user):
        ...
        last_login_label = customtkinter.CTkLabel(card_frame, text=f"Last Login: {user.get('last_login', 'N/A')}", font=("Segoe UI", 12))
        last_login_label.pack(anchor="w", padx=10, pady=2)
    ...
    def _register_initial_admin_threaded(self, user_id, name, login_key):
        try:
            if self.user_system.register_user(user_id, login_key, "admin", name):
                messagebox.showinfo("Success", "Initial admin registered successfully!")
                self.initial_register_frame.pack_forget()
                self.show_main_frame()
            else:
                messagebox.showerror("Error", "Registration failed. User ID might already exist or face data capture failed.")
                self.show_initial_register_frame() # Show initial register frame again
    ...
    def _admin_register_user_threaded(self, user_id, name, login_key, role):
        try:
            if self.user_system.register_user(user_id, login_key, role, name):
                ...
                messagebox.showerror("Error", "Registration failed. User ID might already exist or face data capture failed.")
                self.show_admin_register_frame() # Show admin register frame again
        except Exception as e:
            messagebox.showerror("Error", f"Registration failed: {e}")
            self.show_admin_register_frame() # Show admin register frame again
        finally:
    ...
    def change_login_key_threaded(self):
        old_key = self.old_login_key_entry.get()
        new_key = self.new_login_key_entry.get()
        confirm_new_key = self.confirm_new_login_key_entry.get()

        if not old_key or not new_key or not confirm_new_key:
            messagebox.showerror("Error", "All fields are required.")
            return
        if new_key != confirm_new_key:
            messagebox.showerror("Error", "New login key and confirmation do not match.")
            return
        
        self.show_loading_spinner()
        threading.Thread(target=self._change_login_key_threaded, args=(old_key, new_key)).start()

    def _change_login_key_threaded(self, old_key, new_key):
        try:
            user = self.user_system.get_logged_in_user()
            if not user:
            ...
            if self.user_system.change_login_key(user['user_id'], old_key, new_key):
                messagebox.showinfo("Success", "Login key changed successfully!")
                self.back_to_dashboard()
            else:
                messagebox.showerror("Error", "Failed to change login key. Old key might be incorrect.")
        except Exception as e:
            messagebox.showerror("Error", f"Error changing login key: {e}")
        finally:
            self.hide_loading_spinner()
    ...
    def populate_users_list(self):
        ...
        for user in users:
            last_login_timestamps = self.user_system.get_user_login_timestamps(user['user_id'])
            if last_login_timestamps:
                user['last_login'] = sorted(last_login_timestamps, reverse=True)[0]
            else:
                user['last_login'] = 'N/A'
    ...
    def populate_admin_recent_logins(self):
        for widget in self.admin_recent_logins_frame.winfo_children():
            widget.destroy()
        ...
        for user in all_users:
            ...
            for ts in timestamps:
                all_login_timestamps.append((ts, user['user_id']))
        
        if not all_login_timestamps:
            no_logins_label = customtkinter.CTkLabel(self.admin_recent_logins_frame, text="No recent logins for any user.", font=("Segoe UI", 12))
            no_logins_label.pack(pady=5)
            return

        # Sort by timestamp (most recent first) and take the last 4
        sorted_timestamps = sorted(all_login_timestamps, key=lambda x: x[0], reverse=True)[:4]

        for ts, user_id in sorted_timestamps:
            customtkinter.CTkLabel(self.admin_recent_logins_frame, text=f"User {user_id}: {ts}", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=2)
